...
This document defines the Service Level Objectives (SLOs) for the Gebeta Restaurant OS platform. SLOs are measurable targets that define the expected level of service for critical system components.
...
histogram_quantile(0.95,
  sum(rate(http_request_duration_seconds_bucket{job="gebeta-api"}[5m])) by (le)
) * 1000

# Error rate (last 5 minutes)
sum(rate(http_requests_total{job="gebeta-api",status=~"5.."}[5m]))
  / sum(rate(http_requests_total{job="gebeta-api"}[5m]))

# Realtime propagation P95
histogram_quantile(0.95,
  sum(rate(gebeta_realtime_propagation_seconds_bucket[5m])) by (le)
) * 1000

# KDS queue depth
gebeta_kds_queue_depth

# Payment success rate
sum(rate(gebeta_payment_requests_total{status="completed"}[5m]))
  / sum(rate(gebeta_payment_requests_total[5m]))
```
...
curl -H "Authorization: Bearer $SENTRY_TOKEN" \
  "https://sentry.io/api/0/projects/gebeta/gebeta-api/stats/?stat=errors&resolution=1h"

# Get transaction duration P95
curl -H "Authorization: Bearer $SENTRY_TOKEN" \
  "https://sentry.io/api/0/projects/gebeta/gebeta-api/transactions/?stat=duration_p95"
```
...
lhci collect --url=https://gebeta.app/merchant/dashboard
```
...
- alert: RealtimeCriticalLag
  expr: histogram_quantile(0.95, rate(gebeta_realtime_propagation_seconds_bucket[5m])) > 5
  for: 1m
  labels:
    severity: critical
...
- alert: RealtimeHighLag
  expr: histogram_quantile(0.95, rate(gebeta_realtime_propagation_seconds_bucket[5m])) > 2
  for: 5m
  labels:
    severity: warning
```
...
- alert: PaymentSuccessCritical
  expr: sum(rate(gebeta_payment_requests_total{status="completed"}[5m])) / sum(rate(gebeta_payment_requests_total[5m])) < 0.90
  for: 5m
  labels:
    severity: critical
...
- alert: PaymentSuccessLow
  expr: sum(rate(gebeta_payment_requests_total{status="completed"}[5m])) / sum(rate(gebeta_payment_requests_total[5m])) < 0.95
  for: 5m
  labels:
    severity: warning
```
...
| --------- | --- | ------- |
| Grafana - Production | https://grafana.gebeta.app/d/gebeta-prod-overview | Real-time metrics |
| Sentry - Errors | https://sentry.io/organizations/gebeta | Error tracking |
| Vercel Analytics | https://vercel.com/gebeta/analytics | Frontend performance |
