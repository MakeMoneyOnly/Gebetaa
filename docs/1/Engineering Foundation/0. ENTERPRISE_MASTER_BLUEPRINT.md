...
app.post('/print/receipt', async (req, res) => {
  ...
  try {
    ...
    items.forEach(item => {
      ...
      printer.tableCustom([
        { text: `${item.quantity}x ${item.name_am || item.name_en}`, align: 'LEFT', width: 0.7 },
        { text: `${lineTotal} ብር`, align: 'RIGHT', width: 0.3 },
      ]);
      // Print modifier selections if any
      if (item.modifiers?.length) {
        item.modifiers.forEach(mod =>
          printer.println(`  + ${mod.name_am || mod.name}`)
        );
      }
    });
    printer.drawLine();
...
app.listen(3001, '127.0.0.1', () =>
  console.log('Gebeta Print Server ready on localhost:3001')
);
```
...
export const PlanFeatures = {
  ...
  pro:        ['analytics', 'loyalty', 'inventory', 'staff_scheduling',
                'unlimited_tables', 'unlimited_staff', 'kds_stations'],
...
export async function POST(req: Request) {
  ...
  const isValid = verifyTelebirrWebhook(raw, nonce, timestamp, signature,
...
{!guestIsAuthenticated && orderItems.length > 0 && (
  <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
    ...
    <div className="flex gap-2 mt-3">
      ...
      <TextButton onClick={proceedToPayment}>
        ያለ ሂሳብ ይቀጥሉ / Skip
      </TextButton>
    </div>
  </div>
...
export async function awardLoyaltyPoints(payload: OrderCompletedPayload) {
  ...
  await publishEvent({
    ...
    payload: { guest_id, points, restaurant_id, order_id,
...
export const PlanFeatures = {
  ...
  pro:        ['analytics','loyalty','inventory','staff_scheduling',
               'unlimited_tables','unlimited_staff','kds_5','reports'],
...
*Financial and legal risk. No dependencies. Do these before anything else.*
...
*Standalone — no upstream dependencies. Do in parallel with Sprint 1.*
...
*These are non-negotiable. They do not bend as the platform grows.*
...
*Gebeta Enterprise Master Blueprint v4.1 — Final*  
*March 2026 · Addis Ababa, Ethiopia*  
*All 18 IDE audit questions incorporated · 500 → 10,000+ Restaurants & Cafés*