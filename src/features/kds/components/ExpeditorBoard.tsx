...
export function ExpeditorBoard() {
    ...
    const { role, restaurantId: roleRestaurantId, loading: roleLoading } = useRole(queryRestaurantId);
    ...
    const fetchQueue = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch('/api/kds/queue?station=expeditor&limit=120&sla_minutes=30');
    ...
    const fullyReadyOrders = useMemo(
        () =>
            consolidated.filter(
                order => order.readiness.total > 0 && order.readiness.ready === order.readiness.total
            ),
        [consolidated]
    );
        ...
        <div className="font-manrope flex h-full min-h-0 flex-col bg-gray-50">
            ...
            <div className="mx-8 mt-4 rounded-xl border border-gray-200 bg-white px-4 py-3">
                <div className="flex flex-wrap items-center gap-3 text-sm">
                    <span className="font-semibold text-gray-800">Ready auto-archive (minutes):</span>
                    <input
                        type="number"
                        min={0}
                        max={180}
            ...
            <main data-lenis-prevent className="flex-1 min-h-0 overflow-y-auto px-8 py-6">
                            ...
                            >
                                ...
                                <div className="mt-3 rounded-xl bg-gray-50 p-3">
                                    ...
                                    <div className="mt-2 space-y-1">
                                        {Object.entries(order.readiness.byStation).map(([station, stats]) => (
                                ...
                                <button
                                    disabled={!allReady || handoffOrderId === order.id || !canFinalizeHandoff}
