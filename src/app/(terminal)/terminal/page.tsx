...
export default function TerminalPage() {
    ...
    const loadOverview = useCallback(async () => {
        ...
        try {
            ...
            toast.error(error instanceof Error ? error.message : 'Failed to load terminal workspace');
        } finally {
            setLoading(false);
        }
    }, [deviceToken]);
    ...
    const selectedPaymentOption = useMemo(
        () => (overview?.payment_options ?? []).find(option => option.method === paymentMethod) ?? null,
    ...
    const splitPaidById = useMemo(() => {
        ...
        for (const payment of splitPayload?.split_payments ?? []) {
            ...
            totals.set(
                payment.split_id,
                Number(((totals.get(payment.split_id) ?? 0) + Number(payment.amount ?? 0)).toFixed(2))
            );
        }
        return totals;
    ...
    const capturePayment = async (args: {
        ...
        try {
            ...
            await Promise.all([loadOverview(), selectedOrderId ? loadSplitPayload(selectedOrderId) : null]);
        } catch (error) {
    ...
    if (!deviceToken && !loading) {
        return (
            <div className="flex min-h-screen items-center justify-center p-6">
                <div className="max-w-md rounded-3xl border border-gray-200 bg-white shadow-sm p-8 text-center">
                    <AlertCircle className="mx-auto h-10 w-10 text-amber-300" />
                    <h1 className="mt-4 text-3xl md:text-4xl font-bold tracking-tight text-gray-900">Terminal not paired</h1>
        ...
        return (
            <div className="flex min-h-screen items-center justify-center p-6">
                <div className="max-w-md rounded-3xl border border-red-400/20 bg-red-500/10 p-8 text-center">
                    ...
                    <h1 className="mt-4 text-3xl md:text-4xl font-bold tracking-tight text-gray-900">Wrong device role</h1>
                    <p className="mt-2 text-[15px] text-gray-600">
                        This tablet is paired as {getDeviceTypeLabel(deviceInfo.device_type)}. Re-provision
                        it as a cashier terminal to access `/terminal`.
                    </p>
                </div>
            </div>
        );
            ...
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
                <div>
                    ...
                    <p className="font-medium text-gray-500">
                        Review open orders and settle bills seamlessly without entering waiter workflows.
                    </p>
                </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
                <aside className="relative flex flex-col space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="group flex h-[140px] flex-col justify-between overflow-hidden rounded-[2rem] bg-white p-5 shadow-sm transition-all hover:shadow-md border border-gray-100/60">
                            <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-xs font-medium text-gray-400">Settlement Workflow</p>
                            </div>
                        </div>
                        <div className="group flex h-[140px] flex-col justify-between overflow-hidden rounded-[2rem] bg-white p-5 shadow-sm transition-all hover:shadow-md border border-gray-100/60">
                            <div className="flex items-center gap-2">
                                 ...
                                 <span className="text-[10px] font-bold tracking-widest text-gray-400 uppercase">Print</span>
                            </div>
                            <div>
                                <h3 className="text-3xl font-black tracking-tight text-gray-900 capitalize">
                                    {overview?.device.metadata?.receipt_mode ?? 'Prompt'}
                                </h3>
                                <p className="mt-1 text-xs font-medium text-gray-400">Receipt Setting</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-4 overflow-y-auto px-2 pb-4">
                                ...
                                return (
                                    <button
                                        ...
                                        className={`group relative flex w-full flex-col justify-between overflow-hidden rounded-[2rem] border transition-all hover:shadow-md focus:outline-none ${
                                            isSelected
                                                ? 'border-gray-900 bg-gray-900 p-5 shadow-xl text-white'
                                                : 'border-gray-100 bg-white p-5 hover:border-gray-200'
                                        }`}
                                    >
                                        <div className="flex w-full items-start justify-between">
                                            <div className="flex flex-col items-start gap-1">
                                                <span className={`inline-flex rounded-full px-2 py-1 text-[10px] font-bold tracking-widest uppercase ${isSelected ? 'bg-gray-800 text-gray-300' : 'bg-gray-50 text-gray-500'}`}>
                                                    {table.status.replace(/_/g, ' ')}
                                                </span>
                                            </div>
                                            <div className="text-right">
                                                <p className={`text-[10px] font-bold tracking-widest uppercase ${isSelected ? 'text-gray-400' : 'text-gray-400'}`}>
                                                    Amount Due
                                                </p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 flex w-full items-end justify-between">
                                            <div className="text-left">
                                                <p className="mb-1 text-[10px] font-bold tracking-widest uppercase text-gray-400">Table</p>
                                                <div className="text-4xl font-black tracking-tighter" style={{ fontVariantNumeric: 'tabular-nums' }}>
                                                    {table.table_number}
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className={`text-2xl font-bold tracking-tight ${isSelected ? 'text-emerald-400' : 'text-emerald-600'}`}>
                                                    {Number(table.outstanding_total ?? 0).toFixed(2)}
                                                    <span className={`ml-1 text-sm font-medium ${isSelected ? 'text-emerald-500' : 'text-emerald-500'}`}>ETB</span>
                                                </p>
                                            </div>
                                        </div>
                                        
                                        <div className={`mt-4 flex items-center justify-between border-t ${isSelected ? 'border-white/10' : 'border-gray-50'} pt-4`}>
                                            <div className={`text-xs font-bold ${isSelected ? 'text-gray-400' : 'text-gray-500'}`}>
                                                {table.active_order_count} Active {table.active_order_count === 1 ? 'Order' : 'Orders'}
                                            </div>
                                        </div>
                                    </button>
                                );
                            ...
                            {!loading && (overview?.tables.length ?? 0) === 0 && (
                                <div className="rounded-[2.5rem] border border-dashed border-gray-200 bg-gray-50 p-10 text-center">
                                    <p className="text-base font-semibold text-gray-700">No billable tables.</p>
                                    <p className="mt-1 text-sm text-gray-500">Waiting for new orders...</p>
                                </div>
                            )}
                        </div>
                    </div>
                            ...
                            <div className="space-y-4 overflow-y-auto px-1 pb-4">
                                {tableOrders.map(order => {
                                    ...
                                    >
                                        <div className="flex w-full items-start justify-between">
                                            <div className="flex flex-col items-start gap-1">
                                                <span className={`inline-flex rounded-full px-2 py-1 text-[10px] font-bold tracking-widest uppercase ${isOrderSelected ? 'bg-green-100 text-green-700' : 'bg-gray-50 text-gray-500'}`}>
                                                    {order.status}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 flex w-full items-end justify-between">
                                            <div className="text-left">
                                                <h3 className={`text-2xl font-black tracking-tight ${isOrderSelected ? 'text-green-900' : 'text-gray-900'}`}>
                                            ...
                                            <div className="text-right">
                                                <p className={`text-xl font-black tracking-tight text-emerald-600`}>
                                                    ...
                                                    <span className="ml-1 text-[10px] tracking-widest font-bold text-emerald-500 uppercase">ETB</span>
                                                </p>
                                            </div>
                                        </div>
                                    </button>
                                ...
                                {selectedTable && tableOrders.length === 0 && (
                                    <div className="rounded-[2.5rem] mt-4 border border-dashed border-gray-200 bg-gray-50 p-10 text-center flex flex-col items-center justify-center">
                                        <p className="text-base font-semibold text-gray-700">No active orders.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            {!selectedOrder ? (
                                <div className="flex flex-1 items-center justify-center text-center">
                                    <div className="max-w-xs">
                                        ...
                                        <h3 className="text-2xl font-black tracking-tight text-gray-900">Select an Order</h3>
                                        <p className="mt-3 text-[15px] font-medium text-gray-500 leading-relaxed">
                                            Choose an order from the list to review the full settlement or divide the check evenly among multiple guests.
                                        </p>
                                    </div>
                                </div>
                            ) : (
                                <div className="flex flex-1 flex-col space-y-8">
                                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                                        <div>
                                            <p className="mb-1 text-[10px] font-bold tracking-widest uppercase text-gray-400">Order Check</p>
                                        ...
                                        <div className="flex min-w-[200px] flex-col items-end justify-center rounded-[2rem] border border-gray-100 bg-slate-50 p-6 text-right shadow-inner">
                                            <p className="text-[12px] font-bold uppercase tracking-widest text-gray-400">Outstanding</p>
                                            <p className="mt-1 text-5xl font-black tracking-tighter text-emerald-600" style={{ fontVariantNumeric: 'tabular-nums' }}>
                                                {fullOrderRemaining.toFixed(2)}
                                            </p>
                                            <span className="text-sm font-bold tracking-widest text-emerald-500 mt-1 uppercase">ETB</span>
                                        </div>
                                    </div>

                                    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px] flex-1">
                                         <div className="flex flex-col rounded-[2rem] border border-gray-100 bg-slate-50 p-6 shadow-sm">
                                            <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                                                <h4 className="text-xl font-bold tracking-tight text-gray-900">Split Checks</h4>
                                                <div className="flex flex-wrap items-center gap-2">
                                                    <select
                                                        value={splitGuestCount}
                                                        onChange={e => setSplitGuestCount(Number(e.target.value))}
                                                        className="h-12 w-28 rounded-xl border border-gray-200 bg-white px-4 text-sm font-bold text-gray-900 shadow-sm outline-none transition-all focus:border-gray-300 focus:ring-4 focus:ring-gray-100"
                                                    >
                                                        {[2, 3, 4, 5, 6].map(count => (
                                              ...
                                              {(splitPayload?.splits ?? []).length > 0 ? (
                                                  <div className="space-y-4 pb-4">
                                                       {splitPayload!.splits.map((split, i) => {
                                                            const paid = splitPaidById.get(split.id) ?? 0;
                                                            const remaining = Number(Math.max(0, split.computed_amount - paid).toFixed(2));
                                                            const isPaid = remaining <= 0;
                                                            return (
                                                                <div key={split.id} className="group relative overflow-hidden rounded-[2rem] border border-gray-100 bg-white p-5 shadow-sm transition-all hover:shadow-md">
                                                                     <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                                                          <div className="flex-1 pr-4">
                                                                               <p className="text-lg font-bold tracking-tight text-gray-900">
                                                                                    {split.split_label || `Guest ${split.split_index + 1}`}
                                                                               </p>
                                                                               <div className="mt-3 flex items-center gap-3">
                                                                                    <div className="h-2 w-full max-w-[120px] overflow-hidden rounded-full bg-slate-100">
                                                                                        <div
                                                                                            className="h-full rounded-full bg-emerald-500 transition-all duration-500"
                                                                                            style={{ width: `${Math.min(100, (paid / split.computed_amount) * 100)}%` }}
                                                                                        />
                                                                                    </div>
                                                                                    <p className="text-[12px] font-bold text-gray-400 uppercase tracking-widest">
                                                                                        {paid.toFixed(0)} / {Number(split.computed_amount ?? 0).toFixed(0)} ETB
                                                                                    </p>
                                                                               </div>
                                                                          </div>
                                                                          <button
                                                                              onClick={() => void capturePayment({
                                                                                  splitId: split.id,
                                                                                  amount: remaining || Number(split.computed_amount ?? 0),
                                                                                  label: split.split_label || `Guest ${split.split_index + 1}`
                                                                              })}
                                                                              disabled={capturingId === split.id || isPaid}
                                                                              className={`flex h-12 min-w-[130px] shrink-0 items-center justify-center gap-2 rounded-xl px-5 text-sm font-bold transition-all shadow-sm ${
                                                                                  isPaid
                                                                                      ? 'bg-emerald-50 text-emerald-700 opacity-80 shadow-none'
                                                                                      : 'bg-emerald-600 text-white hover:-translate-y-0.5 hover:bg-emerald-500 hover:shadow-emerald-500/20 shadow-md active:scale-95'
                                                  ...
                                                  <div className="flex h-full min-h-[160px] flex-col items-center justify-center rounded-[2.5rem] border-2 border-dashed border-gray-200 bg-white/50 p-6 text-center text-[15px] font-medium text-gray-500">
                                                      <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-white shadow-sm text-gray-400">
                                                         <UserX className="h-5 w-5" />
                                                      </div>
                                                      <p>No split is configured yet.</p>
                                                      <p className="mt-1 text-sm text-gray-400">Use Split Evenly to distribute the total across guests.</p>
                                                  </div>
                                              )}
                                            </div>
                                         </div>

                                         <div className="flex flex-col justify-between rounded-[2rem] bg-gray-900 border border-gray-800 p-8 text-white shadow-xl lg:h-auto ring-1 ring-white/10 relative overflow-hidden">
                                            ...
                                            <div className="relative z-10">
                                                ...
                                                <h4 className="text-2xl font-black tracking-tight">Close Table</h4>
                                            ...
                                            <div className="relative z-10 mt-8 space-y-5">
                                                <div className="space-y-2">
                                                    <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Settlement Amount</label>
                                                    <div className="relative">
                                                        <span className="absolute inset-y-0 left-0 flex items-center pl-4 text-sm font-bold text-gray-400">ETB</span>
                                                        <input
                                                            value={orderAmountInput}
                                                            onChange={e => setOrderAmountInput(e.target.value)}
                                                            placeholder="0.00"
                                                            className="h-14 w-full rounded-2xl border border-gray-700 bg-gray-800 p-4 pl-14 text-lg font-bold text-white outline-none transition-all placeholder:text-gray-500 focus:border-gray-500 focus:bg-gray-700"
                                                        />
                                                    </div>
                                                </div>
                                                
                                                <div className="space-y-2">
                                                    <label className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Provider Reference <span className="text-gray-500 lowercase tracking-normal">(Optional)</span></label>
                                                    <input
                                                        ...
                                                        className="h-14 w-full rounded-2xl border border-gray-700 bg-gray-800 px-4 text-sm font-bold text-white outline-none transition-all placeholder:text-gray-500 focus:border-gray-500 focus:bg-gray-700"
                                                    />
                                                </div>

                                                <button
                                                    onClick={() => void closeTableSettlement()}
                                                    disabled={capturingId === `table-${selectedTable?.id}`}
