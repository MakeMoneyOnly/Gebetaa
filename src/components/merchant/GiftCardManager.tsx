...
interface GiftCardManagerProps {
    ...
    onCreate: (payload: { initial_balance: number; currency: string; expires_at?: string }) => Promise<void>;
        ...
        <section className="rounded-[2rem] border border-gray-100 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Issue and redeem gift cards directly from merchant ops.</p>
                </div>
                        ...
                        return (
                            <div key={card.id} className="rounded-xl border border-gray-100 p-3">
                                <div className="flex items-center justify-between gap-3">
                                    <div>
                                        <p className="text-sm font-semibold text-gray-900">{card.code}</p>
                                        <p className="text-xs text-gray-500">
                                            {card.currency} {Number(card.current_balance).toFixed(2)} / {Number(card.initial_balance).toFixed(2)}
                                        </p>
                                    </div>
                                ...
                                <div className="mt-3 flex gap-2">
                                    <input
                                        value={redeemValue}
                                        onChange={(event) => setRedeemById(prev => ({ ...prev, [card.id]: event.target.value }))}
                                        inputMode="decimal"
                                    ...
                                    <button
                                        ...
                                        disabled={redeemingId === card.id || card.status !== 'active'}
                                    ...
                                    >
                                        {redeemingId === card.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : null}
                                        Redeem
                                    </button>
                                </div>
                            </div>
