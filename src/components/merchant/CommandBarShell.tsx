...
export function CommandBarShell() {
    ...
    const commands = useMemo<CommandItem[]>(
        () => [
            ...
            { id: 'inventory', label: 'Go to Inventory & Cost', run: () => router.push('/merchant/inventory') },
    ...
    return (
        <>
            <button
                ...
                className="fixed bottom-5 right-5 z-40 inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs font-bold text-gray-700 shadow-lg hover:bg-gray-50"
            >
                <Command className="h-4 w-4" />
                Command Bar
                <span className="rounded-md bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500">Ctrl K</span>
            </button>
                ...
                <div className="fixed inset-0 z-50 bg-black/40 p-4">
                    <div className="mx-auto mt-20 w-full max-w-xl rounded-2xl border border-gray-100 bg-white p-3 shadow-2xl">
                        ...
                        <div className="max-h-72 space-y-1 overflow-y-auto">
                            {filtered.length === 0 ? (
                                <p className="rounded-lg px-3 py-4 text-sm text-gray-500">No commands found.</p>
                            ) : (
                                filtered.map((item) => (
                                    <button
                                        key={item.id}
