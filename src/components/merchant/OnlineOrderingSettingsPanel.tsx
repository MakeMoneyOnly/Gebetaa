        ...
        <div className="rounded-[1.5rem] border border-gray-100 bg-white p-5 shadow-sm">
            ...
            {loading ? (
                ...
                <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                    ...
                    <label className="flex items-center justify-between rounded-xl border border-gray-200 p-3">
                        <span className="text-sm font-medium text-gray-700">Accept scheduled orders</span>
                        <input
                            type="checkbox"
                            checked={settings.accepts_scheduled_orders}
                            onChange={(event) => onChange({ ...settings, accepts_scheduled_orders: event.target.checked })}
                        />
                    </label>
                    <label className="flex items-center justify-between rounded-xl border border-gray-200 p-3">
                        ...
                        <input
                            ...
                            onChange={(event) => onChange({ ...settings, auto_accept_orders: event.target.checked })}
                        />
                    </label>
                    <label className="flex items-center justify-between rounded-xl border border-gray-200 p-3">
                        ...
                        <input
                            ...
                            onChange={(event) => onChange({ ...settings, order_throttling_enabled: event.target.checked })}
                        />
                    </label>

                    <label className="space-y-1 rounded-xl border border-gray-200 p-3">
                        <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Prep Time (minutes)</span>
                        <input
                            type="number"
                            min={5}
                            max={180}
                            value={settings.prep_time_minutes}
                            onChange={(event) => onChange({ ...settings, prep_time_minutes: Number.parseInt(event.target.value, 10) || 30 })}
                            className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm outline-none focus:border-gray-400"
                        />
                    </label>

                    <label className="space-y-1 rounded-xl border border-gray-200 p-3">
                        <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Max Daily Orders</span>
                        <input
                            type="number"
                            min={1}
                            max={5000}
                            value={settings.max_daily_orders}
                            onChange={(event) => onChange({ ...settings, max_daily_orders: Number.parseInt(event.target.value, 10) || 300 })}
                            className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm outline-none focus:border-gray-400"
                        />
                    </label>

                    <label className="space-y-1 rounded-xl border border-gray-200 p-3">
                        <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Service Start</span>
                        <input
                            type="time"
                            value={settings.service_hours.start}
                            onChange={(event) => onChange({ ...settings, service_hours: { ...settings.service_hours, start: event.target.value } })}
                            className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm outline-none focus:border-gray-400"
                        />
                    </label>

                    <label className="space-y-1 rounded-xl border border-gray-200 p-3">
                        <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Service End</span>
                        <input
                            type="time"
                            value={settings.service_hours.end}
                            onChange={(event) => onChange({ ...settings, service_hours: { ...settings.service_hours, end: event.target.value } })}
                            className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm outline-none focus:border-gray-400"
                        />
                    </label>

                    <label className="space-y-1 rounded-xl border border-gray-200 p-3 md:col-span-2">
                        <span className="text-xs font-semibold uppercase tracking-wide text-gray-500">Throttle Limit (orders / 15 min)</span>
                        <input
                            type="number"
                            min={1}
                            max={500}
                            value={settings.throttle_limit_per_15m}
                            onChange={(event) => onChange({ ...settings, throttle_limit_per_15m: Number.parseInt(event.target.value, 10) || 40 })}
                            className="w-full rounded-lg border border-gray-200 px-2 py-1.5 text-sm outline-none focus:border-gray-400"
                        />
                    </label>
                </div>
