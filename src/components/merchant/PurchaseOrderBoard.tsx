        ...
        <section className="rounded-[2rem] border border-gray-100 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Create and track procurement workflows by supplier and status.</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
                    {Object.entries(grouped).map(([lane, laneOrders]) => (
                        <div key={lane} className="rounded-xl border border-gray-100 bg-gray-50/60 p-3">
                            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500">
                                {lane.replace('_', ' ')} ({laneOrders.length})
                            </p>
                            <div className="mt-2 space-y-2">
                                {laneOrders.map((order) => (
                                    <div key={order.id} className="rounded-lg border border-gray-100 bg-white p-2">
                                        <p className="text-xs font-semibold text-gray-900">{order.po_number}</p>
                                        <p className="text-xs text-gray-600">{order.supplier_name}</p>
