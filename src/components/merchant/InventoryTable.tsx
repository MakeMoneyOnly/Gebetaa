...
export function InventoryTable({
    ...
    const [movementByItemId, setMovementByItemId] = useState<
            ...
            <div className="flex items-start justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500">
                        Track ingredient stock, reorder levels, and quick stock updates.
                    </p>
                </div>
                ...
                <div className="mt-4 overflow-x-auto">
                    <table className="min-w-full border-collapse text-sm">
                        <caption className="sr-only">
                            Inventory stock and quick adjustment table
                        </caption>
                        <thead>
                        ...
                        <tbody>
                            {items.map(item => {
                                const movementState = movementByItemId[item.id] ?? {
                                    qty: '',
                                    reason: '',
                                };
                                ...
                                return (
                                    <tr key={item.id} className="border-b border-gray-50 align-top">
                                        <th
                                            scope="row"
                                            className="py-3 pr-3 text-left font-semibold text-gray-900"
                                        >
                                            {item.name}
                                            <p className="text-xs font-medium text-gray-500">
                                                {item.sku ?? 'No SKU'} · {item.uom}
                                            </p>
                                        </th>
                                        ...
                                        <td className="py-3 pr-3">
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                />
                                                {(['in', 'out', 'waste'] as const).map(
                                                    movementType => (
                                                        <button
                                                            key={movementType}
                                                            type="button"
                                                            disabled={
                                                                movementSubmittingId === item.id
                                                            }
                                                            onClick={() => {
                                                                const qty = Number(
                                                                    movementState.qty
                                                            ...
                                                            className="h-8 rounded-lg border border-gray-200 px-2 text-[11px] font-semibold tracking-wide text-gray-700 uppercase disabled:opacity-50"
                                                        >
                                                            {movementType}
                                                        </button>
                                                    )
