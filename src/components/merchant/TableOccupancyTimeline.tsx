...
export function TableOccupancyTimeline({ buckets }: TableOccupancyTimelineProps) {
    ...
    return (
        <div className="rounded-[2rem] bg-white p-6 shadow-sm">
            <h3 className="text-lg font-bold text-gray-900">Occupancy Timeline</h3>
            <p className="text-xs text-gray-500 mt-1">Recent open vs close session flow</p>
            ...
            <div className="mt-5 grid gap-3">
                {buckets.map((bucket) => {
                    ...
                    return (
                        <div key={bucket.label} className="grid grid-cols-[80px_1fr_1fr] gap-3 items-center">
                            <span className="text-xs font-semibold text-gray-500">{bucket.label}</span>
                            <div className="h-7 rounded-lg bg-emerald-50 flex items-center px-2">
                                <div className="h-3 rounded bg-emerald-500" style={{ width: `${openWidth}%` }} />
                                <span className="ml-2 text-[11px] font-semibold text-emerald-700">{bucket.opens}</span>
                            </div>
                            <div className="h-7 rounded-lg bg-gray-100 flex items-center px-2">
                                <div className="h-3 rounded bg-gray-700" style={{ width: `${closeWidth}%` }} />
                                <span className="ml-2 text-[11px] font-semibold text-gray-700">{bucket.closes}</span>
                            </div>
                        </div>
                    );
                })}
