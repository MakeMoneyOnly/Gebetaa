...
export async function getEsooraConfig(
    ...
    if (error || !partner) {
        console.error('[Esoora] Failed to get partner config:', error);
        return null;
    }
...
export async function createOrder(
    ...
    try {
        ...
        console.error('[Esoora] createOrder error:', errorMessage);
        return {
            success: false,
            partner_order_id: order.partner_order_id,
            status: 'failed',
...
export async function updateOrderStatus(
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        console.error('[Esoora] updateOrderStatus error:', errorMessage);
...
export async function getDeliveryFee(
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        console.error('[Esoora] getDeliveryFee error:', errorMessage);
        return {
            success: false,
            distance_km: 0,
            fee_santim: 0,
...
export async function sendStatusWebhook(
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        console.error('[Esoora] sendStatusWebhook error:', errorMessage);
        return { success: false, error: errorMessage };
    }
}
...
export function parseWebhookEvent(rawBody: string): {
    ...
    try {
        ...
        console.error('[Esoora] Failed to parse webhook:', rawBody);
        return null;
    }
}
